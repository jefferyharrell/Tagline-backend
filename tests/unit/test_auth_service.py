...
def test_refresh_tokens_success(auth_service, token_store, settings, monkeypatch):
    ...
    monkeypatch.setattr(auth_service, "validate_token", lambda token, token_type: {"type": "refresh"})
    # Patch revoke_refresh_token to just mark revoked
    monkeypatch.setattr(auth_service, "revoke_refresh_token", lambda token: token_store.revoke_refresh_token(token))
    # Now refresh
