...
def test_filesystem_provider_env(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        monkeypatch.setenv("FILESYSTEM_PHOTO_STORAGE_PATH", tempdir)
...
def test_missing_filesystem_path(monkeypatch):
    """Missing FILESYSTEM_PHOTO_STORAGE_PATH raises ValidationError."""
    monkeypatch.setenv("STORAGE_PROVIDER", "filesystem")
    monkeypatch.delenv("FILESYSTEM_PHOTO_STORAGE_PATH", raising=False)
    get_settings.cache_clear()
    ...
    assert "FILESYSTEM_PHOTO_STORAGE_PATH" in str(excinfo.value)
...
def test_unsupported_storage_provider(monkeypatch):
    ...
    monkeypatch.setenv("FILESYSTEM_PHOTO_STORAGE_PATH", "/tmp")  # Required for Settings
    get_settings.cache_clear()
